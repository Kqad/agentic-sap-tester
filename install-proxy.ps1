# install.ps1 â€” One-line installer for codebase-memory-mcp (Windows).
#
...
try {
    Invoke-WebRequest -Uri $Url -OutFile "$TmpDir\$Archive" -UseBasicParsing -Proxy "http://rb-proxy-apac.bosch.com:8080/" -ProxyUseDefaultCredentials
} catch {
...
try {
    Invoke-WebRequest -Uri $ChecksumUrl -OutFile "$TmpDir\checksums.txt" -UseBasicParsing -Proxy "http://rb-proxy-apac.bosch.com:8080/" -ProxyUseDefaultCredentials
